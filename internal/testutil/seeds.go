...
// tenant-scoped master row with an explicit id so tests can reference deterministic
// identifiers. Intended only for _test.go callers.

// SeedTenant inserts (or ignores on conflict) a tenant row.
...
// SeedMapel inserts a subject row.
...
// SeedRuang inserts a room row with a placeholder password hash.
...
// SeedSoalPackage inserts a soal-package metadata row.
...
// SeedExamSession inserts a session row. status must satisfy the status CHECK constraint.
